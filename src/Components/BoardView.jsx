...
export default function BoardView({delay=0}) {
    ...
    useEffect(() => {
        ...
        const again = setTimeout(reload, delay)
...
return (
        ...
        <div className="flex flex-col gap-3">
            ...
            <div className={`text-white text-xl x-sm:text-2xl sm:text-4xl text-center p-2 bg-primary rounded-xl font-mono border-4 border-black tracking-widest ${isLoading ? "animate-pulse -z-10": "animate-none"}`} onClick={() => {if (board && delay) reload()}}>
                {!error && 
                (board && board.name) ||
                (!board && <p>loading</p>)}

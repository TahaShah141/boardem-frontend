...
export default function Navbar({isOpen, setOpen}) {
  ...
  return (
    ...
    <div className="hidden justify-between items-center p-7 bg-primary text-white md:px-10 sm:flex border-b-2 border-black">
        <NavLink to="/" className="text-3xl font-bold md:text-5xl">Board<span className="text-red-500 inline-block -rotate-12 translate-x-1 -translate-y-1">em</span></NavLink>
    ...
    <div className="relative flex justify-between items-center px-7 py-5 bg-primary text-white md:px-10 sm:hidden border-b-2 border-black z-0">
        <NavLink onClick={() => setOpen(false)} to='/' className="text-3xl font-bold md:text-5xl">Board<span className="text-red-500 inline-block -rotate-12 translate-x-1 -translate-y-1">em</span></NavLink>
        <button onClick={() => setOpen(!isOpen)}className="text-white w-10 h-10">
          ...
          <svg className=" w-full h-full" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M3,6H21V8H3V6M3,11H21V13H3V11M3,16H21V18H3V16Z" fill="currentColor"/></svg>}
        </button>
        {isOpen && (
        <div className="absolute top-full left-0 my-3 w-full flex justify-center animate-popup">
          {user && (<div className="bg-primary rounded-3xl border-4 border-neutral-900 py-3 text-lg flex justify-center items-center gap-10 px-5">
          <NavLink className="nav-link" onClick={() => setOpen(false)} to="/">Home</NavLink>
          <NavLink className="nav-link" onClick={() => setOpen(false)} to="/boards">Boards</NavLink>

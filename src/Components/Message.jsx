...
export const Message = ({ message, editable=false }) => {
    ...
    const confirmation = (
        <div className={`${editable? "group-hover:flex": ""} hidden gap-1 x-sm:gap-2 sm:gap-3`}>
            ...
            <button className='w-7 h-7 p-1 x-sm:w-9 x-sm:h-9 sm:h-11 sm:w-11 sm:p-2 rounded-full bg-black bg-opacity-10 hover:bg-opacity-25 hover:scale-110 focus:bg-opacity-25 focus:scale-110  hover:bg-violet-600 focus:bg-violet-600' onClick={(e) => {e.stopPropagation(); cancelAction()}}><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z" fill='currentColor'/></svg></button>
        </div>
    )

    return (
        <div className="group flex flex-col x-sm:gap-2 bg-primary p-3 x-sm:p-5 rounded-md text-white border-4 border-violet-600 hover:border-violet-500 w-full" onMouseLeave={resetStates}>
